...
| ROM C Function | Lines | QuickMUD | Status | Notes |
|----------------|-------|----------|--------|-------|
| `do_put()` | 346-494 | ✅ `obj_manipulation.py::do_put()` (line 52) | ✅ **AUDITED** | PUT-001..003 fixed (TO_ROOM messages, WEIGHT_MULT check, pit timer); 15/15 integration tests across `test_put_room_messages.py`, `test_put_weight_mult.py`, `test_put_pit_timer.py`. **PUT-004 ✅ FIXED (2.14.252):** putting an item into a **carried** container corrupted the carrier's `carry_weight`/`carry_number`. ROM `obj_from_char` subtracts the item's weight/number (`src/handler.c:1678-1679`); `obj_to_obj` then walks the container nesting chain and re-adds `get_obj_number(obj)` + `get_obj_weight(obj) * WEIGHT_MULT(container) / 100` for each *carried* container (`src/handler.c:1971-1984`), so a normal carried bag (WEIGHT_MULT=100) is net-zero and a magic bag reduces weight. The port's local `_obj_from_char` subtracted but `_obj_to_obj` re-added **nothing**, so `put sword bag` (carried bag) permanently dropped `carry_weight` by the sword's weight — a player could stuff a carried bag to slip under the `can_carry_w` movement/encumbrance gate. Fix: `_obj_to_obj` now performs the ROM carried-container re-add walk. Cross-ref **INV-011** (CARRY-WEIGHT-COHERENCE). Surfaced 2026-07-04 by a container-mechanics cold-path probe in the autonomous loop. Test: `tests/integration/test_put_weight_mult.py::test_put_into_carried_bag_is_net_zero_on_carry_weight`. **PUT-005 ✅ FIXED (2.14.298):** `put all <container>` with nothing eligible emitted a non-ROM "You have nothing to put." ROM `do_put`'s put-all branch (`src/act_obj.c:451-491`) is a bare `for` loop with **no `found` flag and no trailing message** — nothing eligible ⇒ ROM prints nothing. Same invented-message class as MOVE-006. Reachable via `put all bag` with only the bag carried (a container can't be put into itself). Fix: `count == 0` returns `""`. Surfaced 2026-07-10 by an object-manipulation hunter sweep, verified against ROM C. Test: `tests/integration/test_put005_empty_put_all_silent.py`. **PUT-006 ⏳ OPEN (MINOR, garbage-input edge):** the container token defaults to the **last** word (`obj_manipulation.py:80` `container_name = parts[-1]`), overridden to `parts[2]` only on `in`/`on`. ROM (`src/act_obj.c:354-358`) uses the **second** token (`arg2`), re-read to the third only if `arg2` is `in`/`on`. Divergence only on 3+ word input lacking `in`/`on`, e.g. `put ring bag junk` → Python targets `junk`, ROM targets `bag`. Very low reach (garbage input). Surfaced 2026-07-10 by an object-manipulation hunter sweep, verified against ROM C. |
| `do_drop()` | 496-657 | ✅ `inventory.py::do_drop()` (line 171) | ✅ **AUDITED** | `drop all`, `drop all.type`, money consolidation, ITEM_MELT_DROP, no-drop, wear-state exclusion all verified; 15/15 integration tests in `test_drop_command.py`. **DROP-001** ⏳ OPEN (MINOR, 2026-07-10): the coin-drop branch gate uses `arg.isdigit()` (`inventory.py:644`) where ROM uses `is_number(arg)` (`src/act_obj.c:511`). `is_number` accepts a leading `-`/`+`, so `drop -5 coins` enters the coin branch in ROM → `atoi("-5")=-5` → `amount <= 0` → **"Sorry, you can't do that."**, whereas Python's `"-5".isdigit()==False` falls through to the item path → **"You do not have that item."** Edge-case input only (negative coin amount). Part of the **`is_number`/`atoi` parity class** — ROM's numeric parsers accept a sign/partial-numeric prefix, Python's `isdigit`/`int()` don't (sibling: WIMPY-002 in ACT_INFO_C_AUDIT). Prefer closing the class once with a shared `rom_is_number`/`rom_atoi` helper applied across do_drop/do_wimpy/do_give rather than piecemeal. Surfaced 2026-07-10 by a give/drop hunter sweep, verified against ROM C. |
| `do_give()` | 659-847 | ✅ `give.py::do_give()` (line 13) | ✅ **AUDITED** | Money paths (gold/silver/coin), equipped-item rejection, carry-limit, bribe trigger, changer NPC return-change, TO_NOTVICT exclusion all verified; 14/14 integration tests in `test_give_command.py`. **(Correction 2.14.150:** the changer gold-branch formula was NOT actually correct despite this ✅ — see GIVE-004 below; the gold branch had a spurious extra `/100`.) **INV-025 PERS reconciliation (2.12.54):** both branches baked the giver name via `act_format(recipient=None)`+`_broadcast_to_room_observers` (no PERS masking), and the **coins branch never dispatched TRIG_ACT** though ROM `:726` is NOT MOBtrigger-suppressed. Converted both to `act_to_room(exclude=victim)` — object branch wrapped in `disable_mobtrigger()` (ROM `:832` MOBtrigger=FALSE; `mp_give_trigger` still fires), coins branch plain so TRIG_ACT fires. Removed the now-unused `_broadcast_to_room_observers` helper. See CROSS_FILE_INVARIANTS_TRACKER INV-025 (`tests/integration/test_inv025_give_pers_sweep.py`). **GIVE-006 ⏳ OPEN (JUDGMENT CALL — parity vs UX):** giving a **worn** item emits "You must remove it first." where ROM emits "You do not have that item." ROM `get_obj_carry` (`src/handler.c:2306`) matches only `wear_loc == WEAR_NONE`, so a worn item returns NULL at `do_give` (`src/act_obj.c:779`) → "You do not have that item." ROM's `if (obj->wear_loc != WEAR_NONE) "You must remove it first."` (`:785-788`) is **dead code** (unreachable — obj is non-NULL only when unworn). Python `mud/commands/give.py:53-58` filters worn via `get_obj_carry` but then `_find_equipped_obj` catches it and emits the ROM-dead-code message. Python's output is arguably **more helpful** (tells the player to remove it), but diverges from ROM's actual output — flagged for a human parity-vs-UX decision rather than a unilateral UX regression. If strict parity is chosen: drop the `_find_equipped_obj` fallback in `do_give` so a worn item returns "You do not have that item." Surfaced 2026-07-10 by an object-manipulation hunter sweep, verified against ROM C. |
...
| ROM C Function | Lines | QuickMUD | Status | Notes |
|----------------|-------|----------|--------|-------|
| `do_buy()` | 2531-2771 | ✅ `shop.py::do_buy()` (line 650) | ✅ **AUDITED** | BUY-001..004 (keeper-voiced refusals + ch.reply) and BUY-005 (haggle on buy) fixed; minor cosmetic gaps deferred. **BUY-011 ✅ FIXED (2.14.254):** the multi-buy stock check (`_collect_matching_stock`) scanned the keeper's ENTIRE inventory and counted non-adjacent duplicates, but ROM `do_buy` (`src/act_obj.c:2667-2686`) counts only a **consecutive run** — it walks `obj->next_content` and `break`s at the first non-matching item. So with a keeper carrying `[lantern, dagger, lantern]`, `buy 2*lantern` in ROM refuses ("I don't have that many in stock.") because the two lanterns are not adjacent, but the port sold both. The interleaved layout is reachable in normal play because Python's `do_sell` head-inserts rather than grouping duplicates. Fix: `_collect_matching_stock` now `break`s on the first non-matching candidate after the selected object, mirroring ROM's consecutive-run count. Surfaced 2026-07-04 by a shop-pricing probe in the autonomous loop. Test: `tests/test_shops.py::test_buy_multi_stock_requires_consecutive_run`. **SELL-005 ✅ FIXED (2.14.255):** `_obj_to_keeper` (shop.py:102-106) guarded cost-standardization with `if existing_cost:`, so when the sold item stacked onto a `cost==0` existing same-proto duplicate it kept its own cost; ROM `obj_to_keeper` (`src/act_obj.c:2424`) assigns `obj->cost = t_obj->cost` **unconditionally** ("keep it standard"), zeroing the sold item's cost. Removed the guard. Surfaced 2026-07-04 as the BUY-011 probe's secondary finding. Test: `tests/test_shops.py::test_obj_to_keeper_standardizes_cost_even_when_existing_is_zero`. |
...
| WEAR-013 | 1635 (`wear_obj` WIELD branch) | equipment.py:438 | ROM's two-hands-free block for wielding a `WEAPON_TWO_HANDS` weapon while a shield is worn ends with a **period**: `"You need two hands free for that weapon."` (`src/act_obj.c:1635`). Python ended it with `"!"`. Note the sibling shield-branch message `"Your hands are tied up with your weapon!"` (`:1606`) correctly keeps its exclamation — only the WIELD-branch punctuation diverged. Found by a source-read sweep of `wear_obj`'s message strings. **Fixed (2.14.286)** — restored the period. Test: `tests/integration/test_equipment_system.py::test_shield_worn_blocks_two_handed_wield_exact_message`. | ✅ FIXED |
| WEAR-014 | 1765-1777 (`equip_char` alignment zap, src/handler.c) | equipment.py:112-176 (`_can_wear_alignment` + all 4 wear/wield/hold/light sites) | ROM's alignment zap lives in `equip_char`: when an evil PC dons an ITEM_ANTI_EVIL object (or good+ANTI_GOOD / neutral+ANTI_NEUTRAL) it emits `act("You are zapped by $p and drop it.", TO_CHAR)` + `act("$n is zapped by $p and drops it.", TO_ROOM)` then `obj_from_char(obj); obj_to_room(obj, ch->in_room)` — the item is **dropped to the floor**, with `$p` = the object short_descr. Python returned a generic `"You are zapped by the item and drop it."` (literal "the item", no `$p`), emitted **no** TO_ROOM broadcast, and **left the item carried** (a state divergence: the item stayed in inventory instead of lying on the ground). **Fixed (2.14.287)** — added `_zap_align(ch, obj)` (drops via `_obj_from_char` + `room.add_object`, room message, `$p` short_descr wording) and routed all four zap sites through it. **Residual (minor, documented):** ROM prints the "You wear $p ..." line before the zap (the zap is in equip_char, called after the wear message); Python checks alignment up-front and emits only the zap line — cosmetic ordering, observable *state* now matches. Test: `tests/test_player_equipment.py::TestAlignmentRestrictionsAndCursedItems::test_alignment_zap_drops_item_to_room_with_object_name`. | ✅ FIXED |
| WEAR-015 | 1401-1677 (`wear_obj` flag dispatch order) | equipment.py:262 (`_wear_obj` HOLD branch) | ROM `wear_obj` checks wear flags in **bit order**, so every armor/shield slot (WEAR_FINGER..WEAR_WRIST + WEAR_SHIELD — all bits below HOLD's 1<<14) is dispatched BEFORE the HOLD branch (HOLD is at `:1670`, second-to-last, just before FLOAT). An object flagged both an armor slot and HOLD is therefore worn on the armor slot, not held. Python's `_wear_obj` checked the HOLD flag first, so such an item was wrongly held. **Unreachable in stock data** (no stock object combines the flags — verified by scanning the area JSON) but the engine loads arbitrary custom areas, so the dispatch order must match ROM. Found by hunter-agent sweep of `wear_obj`'s dispatch order. **Fixed (2.14.288)** — added `_PRE_HOLD_WEAR_MASK` and guarded the HOLD branch with `and not (wear_flags & _PRE_HOLD_WEAR_MASK)`, so an item with an armor flag falls through to `_get_wear_location`. (WIELD, bit 13, is item_type-dispatched separately and excluded from the mask — the WEAPON-type vs ITEM_WIELD-flag dispatch difference is a separate latent edge, noted but not stock-reachable.) Test: `tests/test_player_equipment.py::test_hold_flag_yields_to_armor_slot_per_rom_flag_order`. | ✅ FIXED |

...
| `RECALL-003` | MINOR | src/act_move.c:1569-1573 — `if (IS_NPC(ch) && !IS_SET(ch->act, ACT_PET)) { send_to_char("Only players can recall.", ch); return; }` | `mud/commands/session.py:do_recall` (~`:352`) | **`do_recall` NPC gate returned "" silently and keyed on `master` instead of the ACT_PET flag.** ROM `send_to_char("Only players can recall.")` then returns — it emits the message (visible to a switched immortal or any NPC path with a descriptor), and it blocks on the `ACT_PET` flag. Python returned `""` (a comment even claimed "ROM returns silently" — a misreading) and gated on `master is None`, so (a) the block message never appeared and (b) a charmed non-pet mob (master set, no ACT_PET) would wrongly recall. Note the **skill** handler `mud/skills/handlers.py:recall` was already correct; only the dispatched **command** `do_recall` (session.py) had the bug. Surfaced 2026-07-09 by a source-read sweep of do_recall. **Fix (2.14.285):** gate on `int(ch.act) & int(ActFlag.PET)` and return `"Only players can recall."`. Tests: `tests/test_skill_recall_rom_parity.py::test_do_recall_command_blocks_non_pet_npc_with_message` (+ corrected the stale `== ""` assertion in `test_recall_train_commands.py::test_recall_npc_blocked`). | ✅ FIXED (2.14.285) |
...
| `LOCK-001` | MINOR | src/act_move.c:627-656 (`do_lock` container arm), :761-791 (`do_unlock` container arm) | `mud/commands/doors.py:do_lock` / `do_unlock` (container branches) | **`do_lock`/`do_unlock` container arm checked `CONT_CLOSEABLE` first — a check ROM does NOT have in these two handlers, contradicting the "✅ 100% COMPLETE" rows above.** Unlike `do_open`/`do_close` (which DO check `CONT_CLOSEABLE` after `CONT_CLOSED` — MOVE-008), ROM's `do_lock`/`do_unlock` container branches have **no** closeable check at all; their first guard is `!IS_SET(value[1], CONT_CLOSED)` → `"It's not closed."`. The port inserted `if not CLOSEABLE: return "You can't do that."` as the first guard, so a common **open non-closeable** container (`value[1] == 0` — belt pouches, packs, key rings; **98** such protos ship in the stock areas, e.g. `mahntor.json`) returned "You can't do that." on `lock`/`unlock` where ROM returns "It's not closed." Same stale-false-✅ class as MOVE-008 (which fixed the sibling open/close arm but not lock/unlock). Surfaced 2026-07-10 by an autonomous-loop source-read sweep of the door commands. **Fix (2.14.289):** removed the spurious `CLOSEABLE` guard from both container arms, restoring ROM's `CLOSED → key → has_key → LOCKED` order. Test: `tests/integration/test_lock001_container_closeable_guard.py` (2: lock + unlock of an open non-closeable pouch → "It's not closed."). | ✅ FIXED (2.14.289) |
| `LOCK-002` | MINOR | src/act_move.c:637 (`do_lock` container), :773 (`do_unlock` container) — `obj->value[2] < 0` | `mud/commands/doors.py:do_lock` / `do_unlock` (container branches) | **Container key guard used `value[2] <= 0` instead of ROM's `< 0`, so a keyless container reported "It can't be [un]locked." instead of "You lack the key."** ROM gates "It can't be [un]locked." on `obj->value[2] < 0`; key vnum **0** (a keyless container) is NOT `< 0`, so it falls through to `has_key(ch, 0)` — which never matches (no real object protos to vnum 0) — yielding "You lack the key." The port's `<= 0` short-circuited at 0, so **14** stock **closed keyless** containers (`value[2] == 0`, e.g. `hitower.json`) gave the wrong message. The portal sibling arm one branch up already uses `< 0`, proving the intent. Surfaced 2026-07-10 alongside LOCK-001. **Fix (2.14.290):** `value[2] < 0` in both container arms. Test: `tests/integration/test_lock002_container_key_threshold.py` (2: lock + unlock of a closed keyless chest → "You lack the key."). | ✅ FIXED (2.14.290) |
| `LOCK-003` | MINOR (latent) | src/act_move.c:669 (`do_lock` door), :805 (`do_unlock` door) — `pexit->key < 0` | `mud/commands/doors.py:do_lock` / `do_unlock` (door branches, `key_vnum <= 0`) | **Door key guard uses `key_vnum <= 0` where ROM uses `pexit->key < 0` — same class as LOCK-002 but on the EXIT arm. Unreachable in stock data (all 9234 JSON exit `key` fields are `-1` for "no key", never `0`), so `<= 0` and `< 0` agree today.** Filed for source-faithfulness (WEAR-016/017 pattern): a future area authored with exit `key: 0` would diverge (ROM → "You lack the key.", Python → "It can't be [un]locked."). Not fixed — no reachable case and low risk; revisit if custom data introduces a `key: 0` exit. Surfaced 2026-07-10 alongside LOCK-001/002. | ⏳ OPEN (latent — unreachable in stock data) |
...
**MOVE-009 ✅ FIXED (2.14.270) — `do_flee` omitted move_char's "$n leaves $T."/"$n has arrived." broadcasts.** ROM `do_flee` (`src/fight.c:3002`) escapes via `move_char(ch, door, FALSE)`, which broadcasts `act("$n leaves $T.", ch, NULL, dir_name[door], TO_ROOM)` to the fled-from room and `act("$n has arrived.", ...)` to the destination (`src/act_move.c:196-202`), both gated on `!AFF_SNEAK && invis_level < LEVEL_HERO`. Python `mud/commands/combat.py:do_flee` re-implemented the move inline (`was_in.remove_character` / `new_room.add_character`) and emitted only the `"$n has fled!"` line (FIGHT-062), so bystanders in neither room saw the fleer leave or arrive. **Fix:** emit `$n leaves $T.` to `was_in` before removal and `$n has arrived.` to `new_room` after add, both through `act_to_room` (PERS-masked per witness) and gated by the same `show_movement` (sneak/wizinvis) predicate `move_character` uses. Arrival `do_look` was already present (line ~863). Test: `tests/integration/test_move009_flee_leave_arrive_broadcast.py` (2 — leave+arrive delivered; sneaking fleer suppresses both). Verified red before fix, green after. **Remaining → MOVE-010 (OPEN):** ROM `move_char` (`src/act_move.c:206-234`) also runs the follower cascade — charmed followers in the fled-from room who are standing (and, if prone/charmed, first stand) follow the fleer through the same door. The inline flee move still omits this, so a fleeing master abandons charmed pets that ROM would drag along. Lower priority (edge case — charmed followers standing at flee time); its faithful fix wants full delegation to `move_character`, so filed separately.
| `do_lock()` | 571-702 (131) | 258-334 (76) | **100% (23/23)** | ✅ COMPLETE (see LOCK-001, LOCK-002) |
| `do_unlock()` | 706-837 (131) | 337-413 (76) | **100% (21/21)** | ✅ COMPLETE (see LOCK-001, LOCK-002) |

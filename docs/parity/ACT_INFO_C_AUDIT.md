...
| `do_wimpy()` | 2800-2831 | ✅ `mud/commands/remaining_rom.py` | ✅ **100% COMPLETE** | P2 | Set wimpy (flee threshold). **WIMPY-001** ✅ FIXED (2026-06-20): ROM uses `wimpy = atoi(arg)` (`src/act_info.c:2811`) — non-numeric input → 0, NOT rejected; Python returned the invented "Wimpy must be a number." Now mirrors ROM atoi (non-numeric → 0 → "Wimpy set to 0 hit points."). Surfaced 2026-06-19 probing `remaining_rom.py`; the row was previously "0 gaps". Test: `tests/test_player_wimpy.py::TestWimpyEdgeCases::test_wimpy_non_numeric_sets_zero_like_rom_atoi` + `::test_wimpy_invalid_input_overwrites_to_zero_not_preserved`. See INFO_DISPLAY_AUDIT.md **WIMPY-002** ⏳ OPEN (MINOR, 2026-07-10): ROM `atoi("12x")` returns `12` (parses the leading numeric prefix), but Python's `int(args.split()[0])` raises `ValueError` on `"12x"` → falls back to `0`. So `wimpy 12x` sets wimpy 12 in ROM, 0 in Python. Part of a broader "ROM `atoi` accepts a partial-numeric prefix, Python `int()` does not" class (no shared `rom_atoi` helper exists yet). Low reach (junk-suffixed numeric input is rare); pure non-numeric already matches (both → 0, WIMPY-001). Not closed — file first, and prefer a shared `rom_atoi` helper if/when a second reachable site appears. Surfaced by a renderer-sweep hunter, verified against ROM C. |
...
| `do_title()` | 2547-2577 | ✅ `mud/commands/character.py:108` | ✅ **100% COMPLETE!** | P2 | **0 GAPS!** Set character title (8/8 tests passing) 🎉 - See SESSION_SUMMARY_2026-01-08_P2_CHARACTER_COMMANDS_COMPLETE.md **TITLE — INTENTIONAL DIVERGENCE (do not "fix" back), 2026-07-10:** ROM `src/act_info.c:2562-2564` computes `i=strlen(argument)` then tests `argument[i-1]=='{' && argument[i-2]!='{'` with **no `i>1` guard**, so a one-char `"{"` argument reads `argument[-1]` (out-of-bounds / undefined behavior). Python `mud/commands/character.py` guards with `if i > 1`, so `title {` yields title `"{"` where ROM's UB typically nulls index 0 → "Change your title to what?". This is a deliberate refusal to replicate C undefined behavior (per the "we don't replicate UB" rule); the input is degenerate and ROM's result is not well-defined. Surfaced by a renderer-sweep hunter 2026-07-10. |
| `do_description()` | 2579-2656 | ✅ `mud/commands/character.py:138` | ✅ **100% COMPLETE!** | P2 | **2 GAPS FIXED!** Set character description (13/13 tests passing) 🎉 - See SESSION_SUMMARY_2026-01-08_P2_CHARACTER_COMMANDS_COMPLETE.md **DESC-001 ⏳ OPEN (latent — unreachable in ROM's input range), 2026-07-10:** ROM's plain-replace path (`src/act_info.c:2585,2639-2645`) leaves `buf` empty when the `strlen(buf) >= 1024` guard runs (only the `+`-append branch pre-fills `buf` with the existing description), so the guard **never fires on a plain replace** — ROM accepts a plain replace of any length. Python `mud/commands/character.py:213` checks `len(args) >= 1024` (the argument) on plain replace, wrongly returning "Description too long." for a ≥1024-char replace. **Unreachable in ROM:** `MAX_INPUT_LENGTH == 256` (`src/merc.h:124`), so a single `description` command's argument can never reach 1024 chars — ROM and Python behave identically (both accept) within ROM's reachable input range. The `+`-append branch is correct in both (checks the existing description length, matching ROM). Filed for source-faithfulness (LOCK-003 pattern); not fixed — removing the guard drops a harmless safety cap for zero parity benefit. Revisit only if the Python input layer starts accepting single commands ≥1024 chars. Verified against ROM C by a renderer-sweep hunter 2026-07-10. |

### Security/Settings Commands (2 functions)

| ROM C Function | ROM Lines | QuickMUD Location | Status | Priority | Notes |
|----------------|-----------|-------------------|--------|----------|-------|
| `do_password()` | 2833-2925 | ✅ `mud/commands/character.py` | ✅ **100% COMPLETE!** | P1 | **4 GAPS FIXED!** Change password (15/15 tests passing) 🎉 - See DO_PASSWORD_AUDIT.md (archived). **PASSWORD-001 ✅ FIXED (2.14.42):** the wrong-password penalty used `ch.wait = 40` (plain assignment) where ROM `src/act_info.c:2895` uses `WAIT_STATE(ch, 40)` = `UMAX(ch->wait, 40)` — a higher existing wait was lowered to 40. Now `apply_wait_state(ch, 40)`. Surfaced by the ROM-WAIT_STATE-site cross-check (sibling of SAVE-001). Test: `tests/integration/test_password001_wait_state_umax.py` (2). **PASSWORD-002 ✅ FIXED (2.14.291):** two byte-fidelity gaps in `do_password` — the syntax line dropped ROM's trailing period (`src/act_info.c:2889` `"Syntax: password <old> <new>."`) and the wrong-password penalty line had one space where ROM has **two** (`src/act_info.c:2896` `"Wrong password.  Wait 10 seconds."`). Same wrong-message-string class as the batch-2 render fixes. Surfaced 2026-07-10 by a renderer-sweep hunter, verified against ROM C. Test: `tests/integration/test_password002_message_fidelity.py` (2, exact-byte `==`). |
